document.addEventListener('DOMContentLoaded', async function () {
  ...
  function addPetCard(pet) {
    ...
    // Construct the full image URL from the stored path
    const fullImageURL  = pet.pictureUrl; // Use the URL directly
    petCard.innerHTML = `
      <img class="pet-img" src="${fullImageURL}" alt="${pet.name}" />
      <div class="pet-content">
        <div class="pet-info">
          <p class="pet-name"><strong>Pet Name: ${pet.name}</strong></p>
          <p class="pet-location"><strong>Last Seen:</strong> ${pet.areaLastSeen}</p>
        </div>
    ...
    petCard.addEventListener('click', () => {
      window.location.href = `/html/lost-details.html?id=${pet.reuniteId}`; 
    });
  ...
  async function fetchAndUpdatePets() {
    try {
      const response = await fetch('/pets/lost'); // Fetch pets from the server
      if (!response.ok) {
  ...
  function filterPets() {
    const searchValue = searchBox.value.trim(); // Get the search value
...
searchButton.addEventListener('click', filterPets);
  ...
  await fetchAndUpdatePets(); // Await the initial fetch

  // Fetch the pets every few seconds to update the pet list
